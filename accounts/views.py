...
class UserPermissionListView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["all_perms"] = Permission.objects.select_related("content_type").order_by(
...
class UserPermissionUpdateView(PermissionRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["page_title"] = (
            f"Edit Permissions for User: {self.object.get_full_name() or self.object.username}"
        )
        return ctx
...
class UserProfileUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs["current_user"] = (
            self.request.user
        )  # Pass the current user so the form can decide on staff fields
        return kwargs
...
class DashboardView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        laundry_data = Laundry.objects.filter(
        ...
        ctx["total_complaints"] = self._quick_count(
            "complaints", ComplaintTicket.objects
        )
        ctx["unresolved_complaints"] = self._quick_count(
            "unresolved_complaints", ComplaintTicket.objects.filter(resolved=False)
        )

        # Total Sales in the last 7 days
        last_7_days_sales = Invoice.objects.filter(
        ...
        context_data = [
            {
                ...
                "value": Room.objects.filter(is_available=True).count(),
            },
            {
            ...
            },
            {
                ...
                "value": ctx["laundry_total_clothes"],
            },
            {"description": "Total Complaints", "value": ctx["total_complaints"]},
            {
                "description": "Unresolved Complaints",
                "value": ctx["unresolved_complaints"],
            },
            {
                "description": "Total Sales in Last 7 Days",
                "value": f"₦{ctx['total_sales_last_7_days']}",
            },
        ]
...
class StaffSignupView(CreateView):
    ...
    def form_valid(self, form):
        messages.success(
            self.request, "Application submitted. HR will review your account."
        )
        return super().form_valid(form)
...
class EntryListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["has_open_entry"] = (
            TimeEntry.objects.open().filter(user=self.request.user).exists()
        )
        return ctx
