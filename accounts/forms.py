...
class UserProfileForm(UserChangeForm):
    ...
    current_shift = forms.ChoiceField(
        choices=Staff.SHIFT_CHOICES, label="Shift", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if current_user and (
            current_user.is_superuser or current_user.has_perm("accounts.change_role")
        ):
    ...
    def save(self, commit=True):
        ...
        if hasattr(user, "staff_profile"):
            ...
            ):  # field exists only if user is allowed to edit
                staff.role = self.cleaned_data.get("role") or staff.role
            if "department" in self.cleaned_data:
                staff.department = (
                    self.cleaned_data.get("department") or staff.department
                )
            if "current_shift" in self.cleaned_data:
                staff.current_shift = (
                    self.cleaned_data.get("current_shift") or staff.current_shift
                )
            staff.save()
        return user
...
class StaffApprovalForm(forms.ModelForm):
    ...
    class Meta:
        model = Staff
        fields = ["status", "approved_by", "is_active"]
