...
class InvoiceListView(ListView):
    ...
    def get_queryset(self):
        ...
        if q:
            qs = qs.filter(customer__first_name__icontains=q) | qs.filter(
                customer__last_name__icontains=q
            )
        if status == "yes":
...
class InvoiceDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        paid = self.object.payments.aggregate(total=Sum("amount"))["total"] or Decimal(
            "0"
        )
...
class PaymentCreateView(SuccessMessageMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        response = super().form_valid(form)
        paid_total = invoice.payments.aggregate(total=Sum("amount"))[
            "total"
        ] or Decimal("0")
